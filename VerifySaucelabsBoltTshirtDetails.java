...
public class VerifySaucelabsBoltTshirtDetails extends Utility {
	...
	ArrayList<String> expProdDetails = dataread("D:\\Selenium\\Setup\\SwagLabs TestCases\\TestData.xlsx", "Sheet4", 2);
	...
	public void login()
	{
		starttestcase("Verifying Bolt T-Shirt Details");
		...
		for(int i=0; i<Username.size(); i++)
		{
			...
			elementfinder(objhome.SaucelabsBoltTshirt).click();
			...
			markstatus("info", "Testing Bolt T-Shirt Product Details with user" + Username.get(i));
			
			for(int j=0; j<actualProdDetails.size(); j++)
			{
				compare(expProdDetails.get(j), actualProdDetails.get(j), "T-Shirt Details are as expected" + expProdDetails.get(j),
						"T-Shirt Details are not as expected" + expProdDetails.get(j));
			}
