...
public class VerifyFleeceJacket extends Utility {
	...
	ArrayList<String> expProdDetails = dataread("D:\\Selenium\\Setup\\SwagLabs TestCases\\TestData.xlsx","Sheet4",3); 
	...
	public void login()
	{
		starttestcase("Verifying Fleece Jacket Details");
		...
		for(int i=0; i<Username.size(); i++)
		{
			...
			elementfinder(objhome.SaucelabsFleeceJacket).click();
			...
			markstatus("info", "Fleece Jacket Details with user - " + Username.get(i));
			
			for(int j=0; j<actualProdDetails.size(); j++)
			{
				compare(expProdDetails.get(j), actualProdDetails.get(j), "Product details are as expected" + expProdDetails.get(j), "Product details are not as expected + expProdDetails.get(j)");
			}
